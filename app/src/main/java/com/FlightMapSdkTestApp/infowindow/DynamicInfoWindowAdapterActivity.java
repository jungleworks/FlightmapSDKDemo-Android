...
public class DynamicInfoWindowAdapterActivity extends AppCompatActivity implements OnMapReadyCallback {
  ...
  public void onMapReady(@NonNull MapboxMap map) {
    ...
    mFlightmap.getUiSettings().setDeselectMarkersOnTap(false);

    // Add a marker
    marker = addMarker(mFlightmap);
    mFlightmap.selectMarker(marker);

    // On map click, change the info window contents
    mFlightmap.addOnMapClickListener(mapClickListener);

    // Focus on Paris
    mFlightmap.animateCamera(CameraUpdateFactory.newLatLng(PARIS));
  }
  ...
  protected void onDestroy() {
    super.onDestroy();
    if (mFlightmap != null) {
      mFlightmap.removeOnMapClickListener(mapClickListener);
    }
    mapView.onDestroy();
  }
